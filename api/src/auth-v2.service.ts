...
export class AuthV2Service {
 ...
 private async deliverToken(to:string,subject:string,path:string,token:string){const base=process.env.APP_ORIGIN??process.env.WEB_ORIGIN??'http://localhost:3000',url=`${base}/${path}?token=${encodeURIComponent(token)}`;await this.email.send(to,subject,`<p>Gunakan tautan berikut untuk melanjutkan:</p><p><a href="${url}">${url}</a></p><p>Tautan ini bersifat pribadi dan akan kedaluwarsa.</p>`)}
